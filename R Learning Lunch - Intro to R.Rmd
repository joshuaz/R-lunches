...
---
  ...
  The following is pulled from an online class and edited to fit our needs at Hanover. 
  You can learn more here: https://www.coursera.org/learn/probability-intro

</div>
  
  The goal of this lab is to introduce you to R and RStudio. To straighten 
...
Your R Markdown file (this document) is in the upper left panel. The R Markdown is an file that has some extra functionalities (e.g., creating word document, pdfs, html files).You can also access scripts here as well.
...
packages that make our lives easier, and we can use them for free. Valuable packages you will often use include: 
  
- `dplyr`: for data wrangling
- `ggplot2`: for data visualization

This lab also uses the package "statsr" where datasets are already pre-loaded. 
  - `statsr`: for data files and functions used in this course (note: you may need to pull this from github)

Next, you need to install and load the packages in your working environment. We do this with
...
You may have already installed these packages using commands like `install.packages` and `install_github`. Running the next line will search for the package in your librbary and install it if you do not have it installed already. 
...
list.of.packages<- c("dplyr","ggplot2","devtools")
...
R Markdown is a great solution for this problem. And, you already have worked with an R  
Markdown document -- this lab! Going forward type the code for the questions in the code 
chunks provided in the R Markdown (Rmd) document for the lab, and **Knit** the document to
see the results.
...
```
         ...
         sign allows you to focus on just one column of the data (or one variable) For     
         example, Name_DataFrame$Name_Variable.
 ...
 side of the printout to indicate locations within the vector. For example, in the
 arbuthnot$boys vector, 5218  follows [1], indicating that 5218 is the first entry in
 the vector. And if [43] starts a line, then that would mean the first number on that
 line would represent the 43rd entry in the vector.
...
```
         ...
         What you will see are 82 numbers (in that packed display, because we aren't looking at a data frame here), each one representing the sum we're after. Take a
